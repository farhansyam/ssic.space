<x-layouts.admin title="Peserta Kegiatan">
    <div class="mb-6 flex items-center gap-3">
        ...
        <div>
            <h2 class="font-display text-2xl font-semibold text-slate-800">Peserta: {{ $event->title }}</h2>
                    ...
                    @foreach ($registrations as $reg)
                        <tr class="transition hover:bg-slate-50">
                            <td class="px-5 py-3.5 font-medium text-slate-700">
                                {{ $reg->displayName() }}
                                @unless ($reg->user_id)
                                    <span class="ml-1 rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide text-slate-400">Tamu</span>
                                @endunless
                            </td>
                            <td class="px-5 py-3.5 text-slate-500">{{ $reg->displayEmail() }}</td>
                            <td class="px-5 py-3.5">
                                <form
                                    method="POST"
                                    action="{{ route('admin.events.participants.status', [$event, $reg]) }}"
                            ...
                            <td class="px-5 py-3.5">
                                ...
                                @elseif ($reg->status === 'hadir' && ! $reg->user_id)
                                    <span class="text-xs text-slate-400">Tamu tanpa akun</span>
                                @elseif ($reg->status === 'hadir')
                                    ...
                                    @else
                                        <form method="POST" action="{{ route('admin.events.participants.certificate', [$event, $reg]) }}" x-data="{ submitting: false }" @submit="submitting = true" class="flex items-center gap-1.5">
                                            @csrf
